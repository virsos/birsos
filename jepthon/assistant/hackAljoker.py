...
'''
mm = '''
قم بلأنضمام الى قناة فايروس @VV_RSI
'''
...
keyboard = [
  ...
  [
    Button.url("سورس الجوكر 🤡", "https://t.me/VV_RSI")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/en_jq")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        await event.reply(i + "\n\nشكراً لأستخدامك سورس ففايروس❤️. \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك فايروس.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك فايروس.", buttons=keyboard)
